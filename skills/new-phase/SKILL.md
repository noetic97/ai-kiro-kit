...
Scaffold a multi-session implementation phase. Creates an `IMPL-<slug>.md` tracking file
at repo root and updates `.kiro/steering/context.md` to point at it. Use this when a goal
spans more than one session and needs step-level progress tracking across commits and PRs.
...
> Reference: `.kiro/steering/context.md` for current focus and last completed step.
...
2. Update `.kiro/steering/context.md`: set `Last Completed` and `Next Up` to the exact next step
...
```
**Active:** <Phase goal> — see `IMPL-<slug>.md`. Start there at the beginning of each session.
```
...
1. Delete the IMPL file: `git rm IMPL-<slug>.md`
2. Update `.kiro/steering/context.md`: clear Current Focus, update Last Completed to "Phase complete — <slug>"
