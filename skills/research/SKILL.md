...
Plan and document a technical research question. Produces a structured `RESEARCH-<slug>.md`
...
Create `RESEARCH-<slug>.md` where `<slug>` is a short kebab-case label for the topic.
...
- [ ] Any `[decision-only]` items are noted in `.kiro/steering/context.md` or an ADR if significant
...
For every `[backlog]` item in the recommendations table, add it to `BACKLOG.md` now.
