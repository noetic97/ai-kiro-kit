...
if git diff --name-only 2>/dev/null | grep -q "steering/context.md"; then exit 0; fi
if git diff --cached --name-only 2>/dev/null | grep -q "steering/context.md"; then exit 0; fi
